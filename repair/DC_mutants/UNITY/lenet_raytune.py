...
def run_tune(algo, mutant, repeat):
    ...
    ####
    # params["kr3"] = _params["kernel_regularizer"]
    ####
    ...
    results_df.to_csv(
        f"/home/hpo/results/DC_UNITY_|{mutant}|_{algo}_iter_{N_ITER}_time_{TIME_LIMIT}_repeat_{repeat}_top10_{args.top10}.csv"
    )
...
for mutant in os.listdir("./"):
    ...
    if mutant == "lenet_add_weights_regularisation_mutated0_MP_l1_l2_7.py":
        continue

    # if mutant != "lenet_change_learning_rate_mutated0_MP_False_0.001.py":
    #     continue

    print("=" * 30)
    print(mutant)
    for repeat in range(args.start, args.start + args.repeat):
